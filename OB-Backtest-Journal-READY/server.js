...
function setSessionCookie(res, sid) {
  res.setHeader(
    'Set-Cookie',
    `ob_session=${encodeURIComponent(sid)}; Path=/; HttpOnly; SameSite=Lax${
...
async function deleteStoredValue(sid, key) {
  ...
  await supabaseRequest(`kv?${q.toString()}`, {
    method: 'DELETE',
    headers: {
...
app.post('/api/ai', async (req, res) => {
  ...
  if (!cfg || !key) {
    return res.status(503).json({
      error:
        `Server AI provider "${provider}" is not configured. ` +
        `Add its API key to Render environment variables.`
    });
  }
  ...
  try {
    ...
    return res.status(502).json({
      error:
        `AI upstream request failed: ${err.message}`
    });
  }
});
...
app.listen(PORT, () => {
  console.log(
    `OB Journal running on http://localhost:${PORT}`
  );
});
