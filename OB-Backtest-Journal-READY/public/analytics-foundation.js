...
(function(){
  ...
  function accounts(){return Array.isArray(window.phase1Accounts)&&window.phase1Accounts.length?window.phase1Accounts:[]}
  ...
  function allTrades(){return Array.isArray(window.trades)?[...window.trades]:(typeof getFiltered==='function'?[...getFiltered()]:[])}
  function base(){return allTrades().sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))||Number(a.id||0)-Number(b.id||0))}
  ...
  function matches(t){
    ...
    if(state.account==='current'&&accountId()&&aid!==accountId())return false;
  ...
  function render(){
    ...
    const accountOpts=accounts().map(a=>({value:a.id,label:`${a.name} · ${a.type}`}));
    const select=(id,label,opts)=>`<div class="af-field"><label>${label}</label><select id="${id}"><option value="all">All</option>${opts.map(x=>`<option value="${escapeHtml(String(x.value??x))}">${escapeHtml(String(x.label??x))}</option>`).join('')}</select></div>`;
    p.innerHTML=`<div class="af-head"><div><strong>Analytics filters</strong><span>One filter context powers every analytics view</span></div><button id="af-clear" type="button">Clear all</button></div><div class="af-grid">${select('af-account','Account',[{value:'current',label:'Current account'},{value:'all',label:'All accounts'},...accountOpts])}${select('af-symbol','Pair',symbols)}${select('af-strategy','Strategy',strategies)}${select('af-mode','Mode',modes)}${select('af-session','Session',sessions)}${select('af-day','Day',days)}${select('af-tf','Timeframe',tfs)}${select('af-regime','Market regime',regimes)}${select('af-direction','Direction',['Long','Short'])}<div class="af-field"><label>From</label><input id="af-from" type="date" value="${escapeHtml(state.from)}"></div><div class="af-field"><label>To</label><input id="af-to" type="date" value="${escapeHtml(state.to)}"></div></div>`;
    const map={account:'af-account',symbol:'af-symbol',strategy:'af-strategy',mode:'af-mode',session:'af-session',day:'af-day',timeframe:'af-tf',regime:'af-regime',direction:'af-direction'};
