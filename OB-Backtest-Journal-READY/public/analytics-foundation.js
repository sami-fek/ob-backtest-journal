...
(function(){
  ...
  function matches(t){
    ...
    if(state.mode!=='all'&&val(t,'mode','source')!==state.mode)return false;
  ...
  function options(list,field,extra){return [...new Set(list.map(t=>val(t,...field)))].filter(Boolean).sort()}
  function render(){
    ...
    const select=(id,label,field,opts)=>`<div class="af-field"><label>${label}</label><select id="${id}"><option value="all">All</option>${opts.map(x=>`<option value="${escapeHtml(String(x))}">${escapeHtml(String(x))}</option>`).join('')}</select></div>`;
    p.innerHTML=`<div class="af-head"><div><strong>Analytics filters</strong><span>One filter context powers every analytics view</span></div><button id="af-clear" type="button">Clear all</button></div><div class="af-grid">${select('af-symbol','Pair','symbol',symbols)}${select('af-strategy','Strategy','strategy',strategies)}${select('af-mode','Mode','mode',modes)}${select('af-session','Session','session',sessions)}${select('af-day','Day','day',days)}${select('af-tf','Timeframe','timeframe',tfs)}${select('af-regime','Market regime','regime',regimes)}${select('af-direction','Direction','direction',['Long','Short'])}<div class="af-field"><label>From</label><input id="af-from" type="date" value="${escapeHtml(state.from)}"></div><div class="af-field"><label>To</label><input id="af-to" type="date" value="${escapeHtml(state.to)}"></div></div>`;
  ...
  function apply(){
    ...
    if(typeof analyticsRender==='function'&&!window.__obAnalyticsFoundationRendering){window.__obAnalyticsFoundationRendering=true;try{analyticsRender()}finally{window.__obAnalyticsFoundationRendering=false}}
    render();
  }
