...
(function(){
  ...
  function normalize(row){
    ...
    r.accountId=r.accountId||read().accountId||'real-main';
  ...
  async function parseFile(file){
    try{
      ...
      const rows=Array.isArray(d)?d:(Array.isArray(d.trades)?d.trades:[]);
      if(!rows.length)throw new Error('No trades found');
      const incoming=rows.map(normalize);
      const existing=Array.isArray(window.trades)?window.trades:[];
      const ids=new Set(existing.map(t=>String(t.id)));
      const tickets=new Set(existing.filter(t=>String(t.source||'').startsWith('mt5')&&t.ticket).map(t=>`${t.accountId||''}:${t.ticket}`));
      const fresh=[];
      let duplicates=0;
      for(const trade of incoming){
        const ticketKey=trade.ticket?`${trade.accountId||''}:${trade.ticket}`:'';
        if(ids.has(String(trade.id))||(ticketKey&&tickets.has(ticketKey))){duplicates++;continue;}
        fresh.push(trade); ids.add(String(trade.id)); if(ticketKey)tickets.add(ticketKey);
      }
      if(fresh.length){
        window.trades=existing.concat(fresh);
        if(typeof saveTrades==='function')await saveTrades();
        if(typeof render==='function')await render();
      }
      renderPanel();
      showMsg(`MT5 import complete: ${fresh.length} new trade${fresh.length===1?'':'s'} added${duplicates?` · ${duplicates} duplicate${duplicates===1?'':'s'} skipped`:''}.`,'Import complete');
    }catch(e){showMsg('MT5 import failed: '+(e?.message||'Invalid JSON'),'MT5 import failed')}
  }
  function showMsg(msg,title){
    if(typeof window.disciplineShowWarning==='function')window.disciplineShowWarning(msg);
    else window.alert(`${title||'MT5'}\n\n${msg}`);
  }
  function renderPanel(){
    let p=document.getElementById('mt5-panel');
    if(!p){p=document.createElement('div');p.id='mt5-panel';p.className='panel glass';const a=document.getElementById('backup-panel')||document.getElementById('analytics-risk-panel')||document.querySelector('.wrap');a?.parentNode?.insertBefore(p,a?.nextSibling||null)}
    const s=read();
    p.innerHTML=`<h2>MT5 tracking <small>read-only foundation</small></h2><div class="mt5-row"><div><strong>Connection status</strong><span>${esc(s.connected?'Connected':'Not connected')}</span><small>No automated trading. The current bridge stage imports/syncs trade history only.</small></div><div class="mt5-actions"><button id="mt5-import" type="button">Import MT5 JSON</button><button id="mt5-config" type="button">Configure</button><input id="mt5-file" type="file" accept="application/json,.json" hidden></div></div><div class="mt5-fields"><label>Account ID<input id="mt5-account" value="${esc(s.accountId||'real-main')}"></label><label>Terminal/bridge label<input id="mt5-label" value="${esc(s.label||'MT5 Terminal')}"></label></div><div class="mt5-flow">MT5 Terminal → EA / Bridge → Trading System → Supabase → Dashboard</div>`;
    document.getElementById('mt5-import').onclick=()=>document.getElementById('mt5-file').click();
    document.getElementById('mt5-file').onchange=e=>{const file=e.target.files?.[0];if(file)parseFile(file);e.target.value='';};
  ...
  window.obMT5={normalize,importFile:parseFile,render:renderPanel,settings:read};
})();
