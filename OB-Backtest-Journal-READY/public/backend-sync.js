(() => {
  ...
  async function readServerState(storageKey = STORAGE_KEY) {
    try {
      const response = await fetch(`/api/storage/${encodeURIComponent(storageKey)}`, { credentials: 'same-origin', cache: 'no-store' });
  ...
  async function hydrateStorage() {
    ...
    else if (localState) void writeServerState(localState, STORAGE_KEY);
