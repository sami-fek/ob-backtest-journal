...
(function simpleJournalShell(){
  ...
  function label(text){const d=document.createElement('div');d.className='simple-section-label';d.textContent=text;return d}
  function arrangeMode(id){
    const p=pageEls[id];if(!p)return;
    [context,form,log,ai].forEach(x=>move(x,p));analytics.filter(Boolean).forEach(x=>move(x,p));[...discipline,...legacyDiscipline].forEach(x=>move(x,p));
    if(form&&!p.querySelector('[data-simple-label="journal"]')){const d=label('Journal + trade log');d.dataset.simpleLabel='journal';p.insertBefore(d,form)}
    const firstA=p.querySelector('#analytics-panel');if(firstA&&!p.querySelector('[data-simple-label="analytics"]')){const d=label('Analytics');d.dataset.simpleLabel='analytics';p.insertBefore(d,firstA)}
    const firstD=p.querySelector('#discipline-panel,#discipline-dashboard-panel,#discipline-trend-panel,#discipline-breakdown-panel,#vio-summary');if(firstD&&!p.querySelector('[data-simple-label="violations"]')){const d=label('Rule violations');d.dataset.simpleLabel='violations';p.insertBefore(d,firstD)}
  }
