...
(function simpleJournalShell(){
  ...
  function place(p,selector,key,title){const el=panel(selector);if(!el||el.classList.contains('simple-hidden-section'))return;p.querySelectorAll(`[data-simple-label="${key}"]`).forEach(x=>x.remove());if(title)p.appendChild(label(title,key));p.appendChild(el)}
  function arrangeMode(id){
    const p=pageEls[id];if(!p)return;hideUnwantedPanels();
    // Core journal panels intentionally use their own native headings, matching the original journal workflow.
    place(p,'#phase1-context','context','Trading Context');
    place(p,'#simple-checklist-panel','checklist','Pre-Trade Checklist');
    place(p,'#table-wrap','trade-log','');
    place(p,'#f-add','log-a-trade','');
