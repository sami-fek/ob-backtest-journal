...
(function simpleJournalShell(){
  ...
  const style=document.createElement('style');style.id='simple-journal-shell-styles';style.textContent=`
    ...
    .simple-shell .panel{box-shadow:0 12px 32px -22px rgba(30,64,175,.38);margin-bottom:12px}.simple-shell .stats{margin-bottom:12px}.simple-shell .top{display:none}.simple-hidden-tool{display:none!important}.simple-section-label{font-size:10px;font-weight:750;letter-spacing:.06em;text-transform:uppercase;color:var(--ink-faint);margin:18px 2px 7px}
  ...
  function arrangeMode(id){
    ...
    if(form&&!p.querySelector('[data-simple-label="log"]'))p.insertBefore(label('Log A Trade · Journal + Trade Log','log'),form);
