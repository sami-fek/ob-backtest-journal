...
(function simpleJournalShell(){
  ...
  const style=document.createElement('style');style.id='simple-journal-shell-styles';style.textContent=`
    ...
    .simple-shell .panel{box-shadow:0 12px 32px -22px rgba(30,64,175,.38);margin-bottom:12px}.simple-shell .stats{margin-bottom:12px}.simple-shell .top{display:none}.simple-hidden-tool{display:none!important}.simple-section-label{font-size:10px;font-weight:750;letter-spacing:.06em;text-transform:uppercase;color:var(--ink-faint);margin:18px 2px 7px}.simple-log-btn-label{font-size:11px;font-weight:750;color:var(--accent);margin-left:6px}
    @media(max-width:700px){body.simple-journal{padding:0 10px 32px}.simple-shell{padding-top:10px}.simple-head{margin-bottom:9px}.simple-nav{margin-bottom:12px}.simple-nav button{padding:8px 11px}.simple-page-head h1{font-size:19px}.simple-page-head{align-items:flex-start}}
  ...
  // Preserve the original trade-log side peek/lightbox overlays before rebuilding the shell.
  const overlays=[...wrap.querySelectorAll('.backdrop,.peek,.lightbox')];
  overlays.forEach(x=>document.body.appendChild(x));
  ...
  const panel=s=>{const e=document.querySelector(s);return e?.closest('.panel')||e};const move=(el,target)=>{if(el&&target&&el.parentNode!==target)target.appendChild(el)};const unique=xs=>xs.filter((x,i,a)=>x&&a.indexOf(x)===i);
  const stats=existing.find(x=>x.classList?.contains('stats')),equity=panel('#equity'),context=panel('#phase1-context'),checklist=panel('#simple-checklist-panel'),form=panel('#f-add'),log=panel('#table-wrap'),ai=panel('#ai-coach-panel');
  const analytics=unique(['#analytics-panel','#analytics-comparison','#analytics-risk-panel'].map(panel));
  ...
  if(stats)move(stats,pageEls.dashboard);if(equity)move(equity,pageEls.dashboard);
  [context,checklist,form,log,ai].forEach(x=>move(x,pageEls.backtest));
  analytics.forEach(x=>move(x,pageEls.backtest));
  [...discipline,...legacyDiscipline].forEach(x=>move(x,pageEls.backtest));
  ...
  // Keep the original side-peek journal behavior; the shell only relocates its overlays out of .wrap.
  ...
  function arrangeMode(id){
    const p=pageEls[id];if(!p)return;
    [context,checklist,form,log,ai].forEach(x=>move(x,p));analytics.filter(Boolean).forEach(x=>move(x,p));[...discipline,...legacyDiscipline].forEach(x=>move(x,p));
    if(form&&!p.querySelector('[data-simple-label="journal"]')){p.insertBefore(label('Log A Trade · Journal + Trade Log','journal'),form)}
    const firstA=p.querySelector('#analytics-panel');if(firstA&&!p.querySelector('[data-simple-label="analytics"]')){p.insertBefore(label('Analytics · Win Rate · Net R · Avg R','analytics'),firstA)}
    const firstD=p.querySelector('#discipline-panel,#discipline-dashboard-panel,#discipline-trend-panel,#discipline-breakdown-panel,#vio-summary');if(firstD&&!p.querySelector('[data-simple-label="violations"]')){p.insertBefore(label('Rule Violations','violations'),firstD)}
    const firstC=p.querySelector('#phase1-context');if(firstC&&!p.querySelector('[data-simple-label="context"]')){p.insertBefore(label('Trading Context','context'),firstC)}
    const firstCheck=p.querySelector('#simple-checklist-panel');if(firstCheck&&!p.querySelector('[data-simple-label="checklist"]')){p.insertBefore(label('Pre-Trade Checklist','checklist'),firstCheck)}
  }
  ...
  const observer=new MutationObserver(()=>{const i=document.getElementById('analytics-insights-panel');if(i)i.remove();['backtest','demo','real'].forEach(id=>{if(pageEls[id].classList.contains('active'))arrangeMode(id)});['backup-panel','mt5-panel'].forEach(id=>{const x=document.getElementById(id);if(x)x.classList.add('simple-hidden-tool')});});observer.observe(wrap,{childList:true,subtree:true});
